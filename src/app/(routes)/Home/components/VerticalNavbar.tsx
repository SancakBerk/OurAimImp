...
const VerticalNavbar = (): JSX.Element => {
  ...
  return (
    <div className={` ${isDarkMode && "dark"} relative w-full h-full`}>
      <div className="w-[10vw] h-[100%] fixed top-0 left-0  flex flex-col pt-40 items-center gap-y-10 z-40  border-r-2 border-black dark:border-white opacity-90  max-sm:w-full max-sm:h-[8vh] max-sm:flex-row max-sm:p-1 max-sm:gap-x-2  max-sm:items-center ">
            ...
            return (
              ...
              >
                <div
                  ...
                  className="w-full justify-center bg-white gap-2  dark:bg-opacity-0 h-16 flex bg-opacity-60 items-center text-xl text-white p-5 dark:hover:border rounded relative z-50 max-2xl:flex-col  max-xl:text-sm max-sm:h-full max-sm:p-2  "
                >
        ...
        <div className="absolute bottom-[10vh] w-[70%]  flex justify-center items-center flex-col gap-y-10 max-sm:top-0    max-sm:right-0  max-sm:bottom-auto max-sm:w-[40%] max-sm:h-[8vh] max-sm:flex-row  max-sm:gap-x-2  ">
          <ModeButton
            className="w-[70%] max-sm:w-20 max-sm:h-full "
            setDarkModeProp={() => {
              dispatch(setModeToRedux(!isDarkMode));
            }}
          />
          <ButtonComponent
            parentClassName="w-[70%] max-sm:w-auto max-sm:h-full "
      ...
      {verticalNavbarData.map((eachVerticalNavbarData: verticalNavbarType) => {
        ...
        return (
          <div
            ...
            onMouseLeave={() => setVisibleComponentId(null)}
            className={` fixed top-0 left-[10vw] w-[70vw] h-screen text-black shadow-lg 
    transition-all duration-500 ease-in-out max-sm:left-0 max-sm:z-50 
    max-sm:w-full max-sm:h-[80%]
    ${
      isVisible
        ? "translate-x-0 opacity-100 max-sm:translate-y-0"
        : "-translate-x-full opacity-0 max-sm:-translate-y-full"
    }`}
          >
